    ...
    for index, row in instr_df.iterrows():
        ...
        if row["Direction"] == "Buy":
            ...
            for sell in sell_queue:
                tx_amount = int(min(quantity, sell["quantity"]))
                # Store transaction data
                transactions.append({"timestamp": row["Execution_Timestamp"], "security": instr, "quantity": tx_amount, "price": round((price + sell["price"])/2, 2), "from": sell["trader"], "to": trading_account})
                quantity -= tx_amount
                if quantity <= 0:
            ...
            for buy in buy_queue:
                tx_amount = int(min(quantity, buy["quantity"]))
                # Store transaction data
                transactions.append({"timestamp": row["Execution_Timestamp"], "security": instr, "quantity": tx_amount, "price": round((price + buy["price"])/2, 2), "to": buy["trader"], "from": trading_account})
                quantity -= tx_amount
                if quantity <= 0:
